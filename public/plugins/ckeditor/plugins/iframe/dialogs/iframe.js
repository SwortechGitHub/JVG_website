...
(function () {
	...
	function commitValue(iframeNode) {
		var value = this.getValue(),
			...
			isRemove =
				value === "" || (attributeName === "tabindex" && value === false);
	...
	CKEDITOR.dialog.add("iframe", function (editor) {
		...
		return {
			...
			onOk: function () {
				var iframeNode;
				if (!this.fakeImage) iframeNode = new CKEDITOR.dom.element("iframe");
				else iframeNode = this.iframeNode;
				...
				var attributes = editor.plugins.iframe._.getIframeAttributes(
					editor,
					iframeNode,
				);
							...
							children: [
								{
									...
									validate: CKEDITOR.dialog.validate.htmlLength(
										commonLang.invalidHtmlLength.replace(
											"%1",
											commonLang.width,
										),
									),
								...
								},
								{
									...
									setup: function (iframeNode, fakeImage) {
										...
										if (fakeImage) {
											...
											this.setValue(
												(fakeImageAlign && fakeImageAlign.toLowerCase()) || "",
											);
										}
									},
